			...
			for equal_aviso in equals_avisos:
				...
				for index, numbered_group in enumerate(numbered_filtered_equal_ads, start=0):
					...
					if noe == array_reas_size:
						numbered_group[index]['reas'].append(equal_aviso)
						size_already_on_array = True
						self.con_mongo.grouped_number_of_ads_equals.update({"noe" :noe},{'$set' : {"reas":numbered_group[index]['reas']}}) #raw_equal_avisos
